...
namespace Rewired.Demos {
     ...
     * bool _value can be used as an element source.
     * 
    ...
    */
    ...
    public class CustomControllerDemo : MonoBehaviour {
        ...
        [NonSerialized] // Don't serialize this so the _value is lost on an editor script recompile.
        ...
        private float GetAxisValueCallback(int index) {
            // This will be called by each axis element in the Custom Controller when updating its raw _value
            // Get the current _value from the source axis at index
            if(index >= axisValues.Length) return 0.0f;
            return axisValues[index];
        }

        private bool GetButtonValueCallback(int index) {
            // This will be called by each button element in the Custom Controller when updating its raw _value
            // Get the current _value from the source button at index
